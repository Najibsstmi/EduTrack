      ...
      <div className="dpp-screen-only mx-auto min-w-0 max-w-7xl space-y-4">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <h2 className="text-base font-bold text-slate-950">Simpan Perubahan DPP</h2>
              <p className="mt-1 text-sm text-slate-500">
                Semak maklumat yang telah diedit, kemudian simpan sebagai rekod DPP peperiksaan ini.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                onClick={saveReportFromAction}
                disabled={saving}
                className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
              >
                <Save className="h-4 w-4" aria-hidden="true" />
                {saving ? 'Menyimpan...' : 'Simpan Laporan'}
              </button>
              <button
                type="button"
                onClick={openPreview}
                className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-800 hover:bg-indigo-100"
              >
                <Eye className="h-4 w-4" aria-hidden="true" />
                Preview
              </button>
...
function DppSubjectReportAnalysis({
  ...
  const visibleReportRows = activeSubjectKey === 'all' ? reportRows.slice(0, 8) : reportRows
  const reportListTitle = activeSubjectKey === 'all' ? 'DPP Disimpan' : `DPP ${subjectFilterLabel}`
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(360px,0.95fr)]">
        ...
        <div className="min-w-0 rounded-xl border border-slate-200 p-4">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h2 className="font-semibold text-slate-950">{reportListTitle}</h2>
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
            ...
            <div className="grid gap-3">
              ...
              {activeSubjectKey === 'all' && reportRows.length > visibleReportRows.length ? (
                <div className="rounded-xl border border-dashed border-slate-300 p-3 text-sm text-slate-500">
                  Memaparkan {visibleReportRows.length} laporan terkini daripada {reportRows.length} rekod.
                </div>
              ) : null}
            </div>
          ) : (
...
function DppSavedReportCard({ report, onOpenReport, onDuplicateReport }) {
  ...
  const teacherInterventions = getFilledInterventions(report.teacher_interventions).length
  const issueStatement = String(report.issue_statement || '').trim()

  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="text-sm font-bold text-slate-950">{report.subjectName}</div>
          <div className="mt-1 text-xs text-slate-500">
            {getDisplayLevel(report.gradeLabel)} | {report.classLabel} | {report.examLabel}
          </div>
        </div>
        <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-2">
        <DppReportMiniStat label="Punca" value={teacherCauses + studentCauses} />
...
function DppReportMiniStat({ label, value }) {
  return (
            ...
            <div className="grid gap-2 2xl:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_90px_90px_90px_150px_auto]">
              <label className="grid min-w-0 gap-1 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                ...
                <input
                  ...
                  className="min-w-0 rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal normal-case tracking-normal text-slate-900"
                />
              </label>
              <label className="grid min-w-0 gap-1 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
                Sasaran
              ...
              <button
                ...
                aria-label="Padam program scoreboard"
                title="Padam program scoreboard"
                className="inline-flex items-center justify-center rounded-lg border border-rose-200 px-3 py-2 text-rose-700 hover:bg-rose-50"
              >
                <Trash2 className="h-4 w-4" aria-hidden="true" />
              </button>
            </div>
            <label className="mt-2 grid gap-1 text-[11px] font-semibold uppercase tracking-wide text-slate-500">
              Catatan Sokongan
              <textarea
                ...
                className="w-full min-w-0 rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal normal-case tracking-normal text-slate-900"
              />
            </label>
          </div>
        ))}
